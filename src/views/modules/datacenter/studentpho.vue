  ...
  methods: {
    init(id) {
      ...
      this.url = this.$http.adornUrl(
        `/datacenter/schoolstudentimage/uploadlocal/?token=${this.$cookie.get(
          "token"
        )}`
      );
      if (id) {
        this.studentid = id;
        this.$http({
          url: this.$http.adornUrl("/datacenter/schoolstudentimage/list"),
          method: "get",
          ...
          if (data && data.code === 0) {
            ...
            data.data.forEach(element => {
              ...
              file.url = this.$http.adornUrl(
                `/datacenter/schoolstudentimage/pho/image?token=${this.$cookie.get(
                  "token"
    ...
    handleRemove(file, fileList) {
     ...
     // console.log(oldfileList);
      this.$http
        .get(
          this.$http.adornUrl(
            `/datacenter/schoolstudentimage/pho/del/${file.uid}`
          )
        )
        .then(({ data }) => {
          if (data && data.code === 0) {
