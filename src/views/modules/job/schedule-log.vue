<template>
  <div>
    <el-dialog title="日志列表" :close-on-click-modal="false" :visible.sync="visible" width="95%">
      ...
      >
        ...
        <el-table-column header-align="center" align="center" width="180" label="操作">
          <template slot-scope="scope">
            <el-button type="text" size="small" @click="showlog(scope.row.logId)">查看日志</el-button>
          </template>
        </el-table-column>
      </el-table>
      <el-pagination
...
export default {
  ...
  methods: {
    showlog(id) {
      console.log(`/sys/scheduleLog/loginfo/${id}`)
      this.dialogVisible=true
      this.$http({
        url:this.$http.adornUrl(`/sys/scheduleLog/loginfo/${id}`),
        method:"get"
      }).then(({data})=>{
        if(data&&data.code===0){
          this.text=data.data;
