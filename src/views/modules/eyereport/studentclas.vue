<template>
  ...
  <div>
    ...
    <el-table :data="dataList" border style="width: 100%;"  @selection-change="selectionChangeHandle">
      ...
      <el-table-column  v-if="!show" prop="grade_name" header-align="center" align="center" width="80" label="年级名称">
      </el-table-column>
      <el-table-column  v-if="!show" prop="class_name" header-align="center" align="center" label="班级名称">
      </el-table-column>
      <el-table-column prop="student_number" header-align="center" align="center" label="学生ID">
      </el-table-column>
      <el-table-column prop="student_name" header-align="center" align="center" label="学生姓名">
      </el-table-column>
      ...
      <el-table-column fixed="right" header-align="center" align="center" width="150" label="操作" v-if="show">
        <template slot-scope="scope">
          <el-button type="text" size="small" @click="addOrUpdateHandle(scope.row.student_number)">修改</el-button>
          <el-button type="text" size="small" @click="deleteHandle(scope.row.student_number)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
import AddOrUpdate from './studentclas-add-or-update'
export default {
  name: "studentclas",
  props: {
    showbutton: {
  ...
  methods: {
    ...
    deleteHandle (id) {
        ...
        this.$confirm(`确定对[id=${ids.join(',')}]进行[${id ? '删除' : '批量删除'}]操作?`, '提示', {
          ...
          this.$http({
            url: this.$http.adornUrl('/eyereportdata/schoolclassinfo/delete'),
            method: 'post',
 ...
 // 多选
    ...
    getDataList(id) {
      this.clsid=id||0;
      this.$http({
        url: this.$http.adornUrl("/eyereportdata/schoolclassinfo/list"),
        method: "get",
