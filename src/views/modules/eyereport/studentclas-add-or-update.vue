  ...
  <el-dialog :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" :visible.sync="visible">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="80px">
      ...
      <el-form-item label="学生姓名" prop="student_name">
        <el-input v-model="dataForm.student_name" placeholder="学生姓名"></el-input>
      </el-form-item>
    </el-form>
    <el-row>
      <el-tabs>
...
export default {
  ...
  methods: {
    ...
    init(id, clsid) {
      ...
      this.dataForm.student_number = id || 0;
      this.add=id||0;
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.student_number) {
          this.$http({
            url: this.$http.adornUrl(
              `/eyereportdata/schoolclassinfo/info/${
                this.dataForm.student_number
              }`
            ),
            method: "get",
        ...
        {
          this.dataForm.student_number=0;
          this.dataForm.student_name="";
        }
      });
    },
    // 表单提交
    dataFormSubmit() {
      ...
      this.$refs["dataForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/eyereportdata/schoolclassinfo/${
                !this.add ? "save" : "update"
              }`
            ),
            method: "post",
